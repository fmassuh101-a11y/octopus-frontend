      ...
      if (response.ok) {
        ...
        const creatorMap = new Map(creators.map((c: any) => {
          ...
          return [c.user_id, {
            ...
            avatar_url: c.avatar_url || c.profile_photo_url || bioData.profilePhoto || null,
            firstName: bioData.firstName,
            lastName: bioData.lastName,
            email: bioData.email,
            about: bioData.about,
            city: bioData.city || bioData.location,
            country: bioData.country,
            niche: bioData.niche,
            categories: bioData.categories,
            tiktokAccounts: bioData.tiktokAccounts || [],
            instagram: c.instagram || bioData.instagram,
            tiktok: c.tiktok || bioData.tiktok,
            youtube: c.youtube || bioData.youtube,
    ...
    <div className="min-h-screen bg-neutral-950 pb-24">
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-neutral-900 border-t border-neutral-800">
        <div className="flex justify-around py-2">
          ...
          <Link href="/company/chat" className="flex flex-col items-center py-2 px-4 text-neutral-500 hover:text-neutral-400">
            <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-end sm:items-center justify-center animate-fade-in">
          <div className="bg-neutral-900 w-full max-w-lg max-h-[90vh] overflow-y-auto rounded-t-3xl sm:rounded-2xl animate-fade-in-up">
            ...
            <div className="p-6">
              ...
              {/* Redes con LOGO OFICIAL — muestra TikTok, Instagram y YouTube */}
              {(() => {
                const c: any = selectedCreator.creator || {}
                const tk = c.tiktok || c.tiktokAccounts?.[0]?.username
                const hasAny = tk || c.instagram || c.youtube
                if (!hasAny) return null
                return (
                  <div className="mb-6">
                    <h4 className="font-semibold text-white mb-3">Redes Sociales</h4>
                    <div className="space-y-2.5">
                      {tk && <SocialRow net="tiktok" handle={String(tk)} />}
                      {c.instagram && <SocialRow net="instagram" handle={String(c.instagram)} />}
                      {c.youtube && <SocialRow net="youtube" handle={String(c.youtube)} />}
                    </div>
                  </div>
                )
              })()}
      ...
      {/* Crear contrato para un aplicante aceptado */}
      {contractFor && user && (
        <CreateContractModal
          isOpen={!!contractFor}
          ...
          gigId={contractFor.gig_id}
          companyId={user.id}
          creatorId={contractFor.creator_id}
          creatorName={getCreatorName(contractFor.creator)}
        />
      )}
    </div>
  )
