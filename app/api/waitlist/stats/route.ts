...
// SEED: arranca "sembrado" para no verse vacío al promocionar (pedido de Felipe).
const SEED_CREATORS = 43;
const SEED_COMPANIES = 7;
...
export async function GET() {
  try {
    ...
    return NextResponse.json({
      ok: true,
      creators: SEED_CREATORS + creators,
      companies: SEED_COMPANIES + companies,
      goalCreators: 250,
      goalCompanies: 50,
    });
  } catch {
    return NextResponse.json({ ok: true, creators: SEED_CREATORS, companies: SEED_COMPANIES, goalCreators: 250, goalCompanies: 50 });
  }
}
