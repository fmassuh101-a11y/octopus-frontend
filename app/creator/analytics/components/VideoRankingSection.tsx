...
import { getTopPerformers, getUnderperformers, formatNumber } from '@/lib/utils/videoAnalytics'
...
export function VideoRankingSection({ videos, onVideoClick }: VideoRankingSectionProps) {
  ...
  const underperformers = getUnderperformers(videos, 3)
    ...
    <div className="grid md:grid-cols-2 gap-6">
      ...
      <div className="bg-white rounded-2xl p-5 border border-gray-100">
        <div className="flex items-center gap-2 mb-4">
          ...
          <div>
            <h3 className="font-semibold text-gray-900">Necesitan Mejorar</h3>
            <p className="text-xs text-gray-500">Videos con menor rendimiento</p>
          </div>
        </div>

        <div className="space-y-2">
          {underperformers.slice(0, 3).map((video, index) => (
            <VideoCard
              key={video.id}
              video={video}
              rank={index + 1}
        ...
        {underperformers.length === 0 && (
          <p className="text-sm text-gray-500 text-center py-4">
            No hay videos con bajo rendimiento
          </p>
        )}
      </div>
    </div>
